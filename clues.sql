...
WHERE countrycode = (SELECT code FROM countries WHERE name = 'Holy See (Vatican City State)') AND isofficial = TRUE;
...
FROM cities
WHERE name LIKE 'Ser%' AND countrycode != (SELECT code FROM countries WHERE name = 'San Marino');
...
SELECT name
FROM cities
WHERE countrycode = 'BRA' AND id = (SELECT capital FROM countries WHERE name = 'Brazil');
...
FROM cities
WHERE population BETWEEN 91000 AND 91085;
